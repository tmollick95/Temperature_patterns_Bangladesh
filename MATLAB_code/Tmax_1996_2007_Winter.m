...
hold off;
print(gcf, ' Predicted Maximum Temperature of January (1996 - 2007).jpg', '-djpeg', '-r300');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Extract longitude and latitude
longitude = winter1996err(:, 1);
latitude = winter1996err(:, 2);

% Extract temperature data from column 3, 7, 11, ... (January months)
num_columns = size(winter1996err, 2);
january_indices = 3:4:num_columns;

% Calculate the mean of all January months
mean_january_error = mean(winter1996err(:, january_indices), 2);

% Combine longitude, latitude, and mean January temperatures into a new matrix
winter1996prerr = [longitude, latitude, mean_january_error];
...
% Release the plot hold
hold off;
print(gcf, ' Error Map of January (1996 - 2007).jpg', '-djpeg', '-r300');
...
actual_values = winter1996prtemp(:, 3);
error_values = winter1996prerr(:, 3);
...
Z = Mat(26, 3:end)';  % Temperature data, transposed to have columns as different datasets
cal_var = var(Z);
...
grid on;
hold off;
print(gcf, 'Cross Validation Actual vs Predicted Temperature of January (1996 - 2007).jpg', '-djpeg', '-r300');
...
Predicted_temp = winter1996pr(:, 3:end); % Replace with your actual data

% Pre-allocate arrays for storing average temperatures for November to February
numYears = 12; % From 1996 to 2007
